 ...
 * Only runs for "Carbon Negative Local Delivery" shipping type.
 */
export async function syncOrderToStopSuite(order) {
  ...
  try {
    ...
    // 4️⃣ Create Shop Order (with external_reference for fulfillment)
    const shopOrderPayload = {
      ...
      external_reference: `shopify_${order.id}`, // 🔗 ties back to Shopify fulfillment
      route_id: 1234, // optional: your route ID in StopSuite (replace if dynamic)
      service: "Delivery",
    };
